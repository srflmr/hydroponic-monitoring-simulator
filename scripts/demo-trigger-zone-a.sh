...
docker compose exec -T sensor-simulator-zone-a python -c "import json,urllib.request as u; r=u.Request('http://localhost:3006/simulate', data=json.dumps({'param':'ec','value':1.1,'duration_seconds':20}).encode(), headers={'Content-Type':'application/json'}, method='POST'); print(u.urlopen(r).read().decode())"
