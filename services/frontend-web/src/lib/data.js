// Domain data, default configuration, and pure helpers for the
// Hydroponic Monitoring frontend. No Svelte here — just plain JS so it can be
// unit-tested and reused by the sim store and components.
...
// Display ranges per signal (axis bounds for rings + charts).
export const RANGE = {
...
// 10%-of-range band, matching the backend evaluator. level keeps a display heuristic.
...
// Build a view-model for a single reading (value string, colours, gauge fill).
...
// Worst status over ph/ec/temp ONLY (water level is display-only, never affects zone status).
